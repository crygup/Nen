...
export function mountPlayer(actions: {
  ...
  const downloadPercent = el<HTMLOutputElement>("download-percent");
  seek.onpointerdown = () => {
    dragging = true;
  };
  seek.oninput = () => {
  ...
  return (p: Playback) => {
    ...
    downloadPercent.hidden = !download;
    downloadPercent.textContent = download ? `Loaded ${download.percent}%` : "";
